...
<div class="article-holder">
	<form method="post" style="margin:0 auto;width:70%; border:1px solid #cfcfcf;border-radius: 5px;padding:20px;">
		<div>
			<span class="form-title" style="display:block;">Заголовок</span>
			<input type="text" name="title" value="<?=$title;?>" style="padding:5px 10px;width:90%;border: 1px solid #cfcfcf;border-radius: 2px;">
		</div>
		<div>
			<span class="form-article" style="display:block;">Статья</span>
			<textarea name="article"  style="display:block;width:96%;height:200px;border: 1px solid #cfcfcf;border-radius: 3px;"><?=$article;?></textarea>
		</div>
		<button style="margin:10px 0px;height:30px;background: #ca6a96; color:#ffffff;border: 0px;border-radius: 5px ;cursor: pointer;" >Редактировать</button>
	</form>
	<?php echo $msg; ?>
